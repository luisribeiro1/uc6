<?php

// Tipagem dinâmica

$nome = "joão da silva";       // string
$cpf = "12345678";             // string
$idade = 42;                   // integer 
$peso = 69.52;                 // double
$situação = true;              // boolean = true ou false
$estadoCivil = null;           // null
//$Habilidades = array("HTML","CSS","JS","PHP");
$Habilidades = ["HTML","CSS","JS","PHP"];  // array
const PI = 3.14;

//echo gettype($nome);

//echo " A variavel \$idade é do tipo ".gettype($idade)." e o seu valor é {$idade}"; 

$tipo = gettype($idade);
echo "<h3> A variavel \$idade é do tipo  {$tipo}  e o seu valor é {$idade}</h3>";       

# A variavel $nome é do tipo string eo seu valor é  João da Silva

# () funções     funções da linguagem e função do usuário 
# [] array       Array é uma estrutura de dados 
# Dentro do echo {} é usado como interpolação
# Dentro do echo . é usado como concatenaçõa
# Dentro do echo \  é usado como 'escape' que neutraliza a execução do código seguinte
# E o echo produz resultado para visual para o usuário em forma de HTML


